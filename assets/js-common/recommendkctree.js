...
var treeSetting = {
	...
	async: {
		...
		type: "get",
		dataType:"json",
		url:"/server/recommend/admin/recommend/position/list.json",
...
function loadSelectTreeNode(parentId) {
	...
	$.getJSON("/server/recommend/admin/recommend/position/list.json?parentId="+parentId,function(json){
	});
}
//点击左边的树 右边内容的操作
...
function delTreeNode(id,parentId,callback){
	$.post("/server/recommend/admin/recommend/position/delete.json",{id:id},function(json){
		removeNode(id);
		loadTreeNode(parentId,callback);		
	});
}
