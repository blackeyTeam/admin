...
var IMG_MAXCOUN = 1;//最多选中图片张数
var IMG_AJAXPATH = "ajax/ImgUpLoad.ashx";//异步传输服务端位置


var UP_IMGCOUNT = 0;//上传图片张数记录
//打开文件选择对话框
$("body").on("click","#gg_imgfile",function () {
    if ($(".gglookimg").length >= IMG_MAXCOUN) {
        alert("只允许一张，请先移除图片后上传。");
        return;
    }

    var _CRE_FILE = document.createElement("input");
    if ($(".ggimgfile").length <= $(".gglookimg").length) {//个数不足则新创建对象
        _CRE_FILE.setAttribute("type", "file");
        _CRE_FILE.setAttribute("name", "file");
        _CRE_FILE.setAttribute("class", "ggimgfile");
        _CRE_FILE.setAttribute("accept", ".png,.jpg,.jpeg");
        _CRE_FILE.setAttribute("num", UP_IMGCOUNT);//记录此对象对应的编号
        $("#gg_imgfile").after(_CRE_FILE);
    }
    else { //否则获取最后未使用对象
        _CRE_FILE = $(".ggimgfile").eq(0).get(0);
    }
    return $(_CRE_FILE).click();//打开对象选择框
});

//创建预览图，在动态创建的file元素onchange事件中处理
$("body").on("change",'.ggimgfile',function () {
    if ($(this).val().length > 0) {//判断是否有选中图片

        //判断图片格式是否正确
        var FORMAT = $(this).val().substr($(this).val().length - 3, 3);
        if (FORMAT != "png" && FORMAT != "jpg" && FORMAT != "peg" && FORMAT != "JPG" && FORMAT != "PNG") {
            alert("文件格式不正确！！！");
            return;
        }
        ...
        _prevdiv.setAttribute("class", "gglookimg");
        //创建内层img对象
        var preview = document.createElement("img");
        preview.setAttribute("class", "zoomify1");
...
//删除选中图片
$("body").on("click",'.lookimg_delBtn',function () {
    $(".ggimgfile[num=" + $(this).parent().attr("num") + "]").remove();//移除图片file
    $(this).parent().remove();//移除图片显示
});

//删除按钮移入移出效果
$("body").on("mouseover",'.gglookimg',function () {
    if ($(this).attr("ISUP") != "1")
        $(this).children(".lookimg_delBtn").eq(0).css("display", "block");;
});
$("body").on("mouseout",'.gglookimg',function () {
...
$("#btn_ImgUpStart").click(function () {
    ...
    if ($(".gglookimg").length <= 0) {
        alert("还未选择需要上传的图片");
        return;
    }

    //全部图片上传完毕限制
    if ($(".gglookimg[ISUP=1]").length == $(".gglookimg").length) {
        alert("图片已全部上传完毕！");
        return;
    }

    //循环所有已存在的图片对象，准备上传
    for (var i = 0; i < $(".gglookimg").length; i++) {
        var NOWLOOK = $(".gglookimg").eq(i);//当前操作的图片预览对象
        NOWLOOK.index = i;
        //如果当前图片已经上传，则不再重复上传
        ...
        $.ajax({
            ...
            success: function (data) {
                if (data.isok == "1") {
                    ...
                    $(".gglookimg").eq([data.ind]).attr("ISUP", "1");//记录此图片已经上传
                    $(".gglookimg").eq([data.ind]).children(".gglookimg_progress").eq(0).children("div").eq(0).animate({ width: "100%" }, UPTIME, function () {
                        $(this).css("background-color", "#00FF00").text('上传成功');
                    });
                }
                else {//图片未上传成功回调
                    $(".gglookimg")[data.ind].children(".gglookimg_progress").eq(0).children("div").eq(0).css("width", "100%").css("background-color", "red").text("上传失败");
                }
            },
            error: function (err) {
                //服务器连接失败报错处理
            ...
            beforeSend: function () {
                ...
                NOWLOOK.children(".gglookimg_progress").eq(0).css("display", "block");//进度条显示
            }
        });
    }
});